<template>
  <div class="flex h-screen overflow-hidden bg-surface-50 dark:bg-dark-200">
    ...
    <!-- Sidebar -->
    <aside
      class="flex flex-col bg-gradient-to-b from-white via-white to-orange-50/40 dark:from-dark-100 dark:via-dark-100 dark:to-dark-100 border-r border-surface-200 dark:border-dark-50 shadow-[4px_0_24px_rgba(0,0,0,.03)] transition-all duration-200 z-40 lg:relative lg:flex-shrink-0 fixed inset-y-0 left-0"
      :class="[
        sidebarOpen
          ? 'w-60 translate-x-0'
    ...
    >
      <!-- User -->
      <div class="border-b border-surface-200 dark:border-dark-50 p-0.5">
        <div class="relative" ref="userMenuRef">
          ...
          >
            <div class="relative flex-shrink-0">
              <div
                class="absolute -inset-0.5 rounded-full bg-gradient-to-br from-brand-400 to-brand-600 opacity-70"
              />
            ...
            <div v-if="sidebarOpen" class="flex-1 text-left min-w-0">
              ...
              <div class="text-[10px] font-bold text-gray-400 truncate">
                {{ authStore.user?.role }}
              </div>
            </div>
          </button>
              ...
              >
                <div
                  ...
                  :class="
                    isActive
                      ? 'bg-white/20'
                      : 'group-hover:bg-brand-50 dark:group-hover:bg-brand-950/40'
                  "
                >
      ...
      <button
        ...
        class="absolute -right-3 top-20 z-10 grid h-6 w-6 place-items-center rounded-full border border-surface-200 bg-white text-gray-500 shadow-[0_4px_12px_rgba(0,0,0,.1)] transition-all hover:border-brand-300 hover:text-brand-600 dark:border-dark-50 dark:bg-dark-100 dark:text-gray-400"
      >
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      ...
      >
        ...
        <div class="flex items-center gap-2 cursor-pointer" @click="handleLogoClick">
          <AppLogo
            compact
            compact-class="h-8 w-auto shrink-0 object-contain"
            class="shrink-0"
          />
          <p
            class="hidden truncate text-[10px] text-gray-400 dark:text-gray-500 sm:block"
          >
            Mua sắm thả ga, hoàn tiền cực đã
          </p>
        </div>
        ...
        <!-- Avatar -->
        <div class="relative" ref="headerUserMenuRef">
          <button
            @click="headerUserMenuOpen = !headerUserMenuOpen"
            class="block shrink-0 rounded-full ring-2 ring-transparent transition-all hover:ring-brand-200 dark:hover:ring-brand-900"
          >
            <UserAvatar :user="authStore.user" size="sm" />
          </button>
          <Transition name="scale-in">
            <div
              v-if="headerUserMenuOpen"
              class="absolute right-0 top-full z-1000 mt-2 w-48 animate-scale-in rounded-2xl border border-surface-200 bg-white py-1.5 shadow-[0_16px_40px_rgba(0,0,0,.12)] dark:border-dark-50 dark:bg-dark-100"
            >
              <NuxtLink
                to="/settings/profile"
                class="flex items-center gap-2 px-3 py-2 text-sm text-gray-700 hover:bg-brand-50 hover:text-brand-700 dark:text-gray-300 dark:hover:bg-brand-950/40 dark:hover:text-brand-300"
                @click="headerUserMenuOpen = false"
              >
                <UserCircleIcon class="w-4 h-4" /> Hồ sơ
              </NuxtLink>
              <NuxtLink
              ...
              <div
                class="my-1 border-t border-surface-200 dark:border-dark-50"
              ></div>
              <button
                @click="handleLogout"
                class="flex w-full items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20"
              >
                <ArrowRightOnRectangleIcon class="w-4 h-4" /> Đăng xuất
              </button>
            </div>
      ...
      <!-- "Vừa mua" ticker -->
      <PurchaseTicker class="flex-shrink-0" />

      <!-- Page content -->
      <main
        class="flex-1 min-h-0 pt-0"
        :class="
          mainScrollLocked ? 'overflow-hidden flex flex-col' : 'overflow-y-auto'
        "
      >
        <slot />
      </main>
...
const headerUserMenuRef = ref(null);
const notifRef = ref(null);
onClickOutside(userMenuRef, () => (userMenuOpen.value = false));
onClickOutside(headerUserMenuRef, () => (headerUserMenuOpen.value = false));
